...
What the tree is not allowed to touch: Value, Schema, Tuple, Type, FieldDef. If you find yourself reaching for any of those inside a tree function, you're in the wrong layer.
    so key is a wrapper around Value
    FieldDef is basically Serialized Type Metadata(b/c it has TypeID+width info)
    Schema is list of FieldDefs
    Tuple is list of Values 
...
Key = value type 
    compare, serialize, deserialize is always implemented
    free = destructor 
    copy = ?? (doesn't it already have copy?? idk...)
