...
auto Type::IsCoercableFrom(const TypeId type_id) const -> bool {
  switch (type_id_) { //dependent the type of "this" object 
    case INVALID:
      return false;
    case BOOLEAN:
      return true;
    ...
    case BIGINT:
    case DECIMAL:
      switch (type_id) { //dependent on the "other" type_id 
        case TINYINT:
        case SMALLINT:
        case INTEGER:
        case BIGINT:
...
auto Type::GetMinValue(TypeId type_id) -> Value {
  switch (type_id) {
    case BOOLEAN:
      return {type_id, 0};
    case TINYINT:
      return {type_id, BUSTUB_INT8_MIN}; //maybe change the BUSTUB_INT<>_<MIN/MAX> to INT<>_<MIN/MAX>
    case SMALLINT:
