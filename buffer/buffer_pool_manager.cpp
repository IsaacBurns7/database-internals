...
// ---------------------------------------------------------------------------------------------
// Test-only helpers. See the declarations in buffer_pool_manager.h for why these exist.
// ---------------------------------------------------------------------------------------------
auto BufferPoolManager::MakeTestReadGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
  ...
  return ReadPageGuard(page_id, std::move(frame), std::move(replacer), std::move(bpm_latch),
                        std::move(disk_scheduler));
}

auto BufferPoolManager::MakeTestWriteGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
                                            std::shared_ptr<ArcReplacer> replacer,
                                            std::shared_ptr<std::mutex> bpm_latch,
                                            std::shared_ptr<DiskScheduler> disk_scheduler) -> WritePageGuard {
  return WritePageGuard(page_id, std::move(frame), std::move(replacer), std::move(bpm_latch),
                         std::move(disk_scheduler));
}

void BufferPoolManager::SetTestFramePinCount(const std::shared_ptr<FrameHeader> &frame, size_t count) {
  frame->pin_count_.store(count);
}

auto BufferPoolManager::GetTestFramePinCount(const std::shared_ptr<FrameHeader> &frame) -> size_t {
  return frame->pin_count_.load();
}

void BufferPoolManager::SetTestFrameDirty(const std::shared_ptr<FrameHeader> &frame, bool dirty) {
  frame->is_dirty_ = dirty;
}