...
auto ReadPageGuard::GetPageId() const -> page_id_t {
  ENSURE(is_valid_, "tried to use an invalid read guard");
  return page_id_;
}
...
WritePageGuard::WritePageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      bpm_latch_(std::move(bpm_latch)),
      disk_scheduler_(std::move(disk_scheduler)) {
  // DIFFERENCE: exclusive lock() instead of ReadPageGuard's lock_shared() — a WritePageGuard
  // must be the only guard (read or write) holding this frame at a time.
...
void WritePageGuard::Drop() {
    ...
    // DIFFERENCE: unlock() instead of ReadPageGuard's unlock_shared() — a WritePageGuard
    // holds the frame's rwlatch_ in exclusive mode, not shared mode.
